...
Modules: `workspaces` (pills; the active one in the accent color, a dot on those with windows; click to switch), `layout` (the workspace's layout, or the hotkey mode while one is active), `window` (the focused application and title; click to focus the next window), `agents` (AI coding agents, see below; click for details), `vpn` (Tunnelblick: filled dot and tunnel name while connected, judged from the tunnel interfaces without any permission; click connects, choosing among several configurations, or disconnects, through Tunnelblick's scripting, which macOS asks you to allow once), `media` (what is playing in Spotify, Music or a YouTube tab in Chrome, with previous, play/pause and next buttons that send the keyboard's media keys, so they steer whatever player is current; reading the title asks once for permission to automate those apps), `network`, `cpu`, `battery`, `clock`, `settings` (left click opens the settings window, right click offers reload and daemon restart) and `spacer`, a fixed gap you can place anywhere in a group. Scrolling anywhere on the bar switches workspaces. Top and bottom bars are horizontal; left and right bars are vertical and 48 points wide. Changes to `[bar]` apply as soon as the file is saved.
